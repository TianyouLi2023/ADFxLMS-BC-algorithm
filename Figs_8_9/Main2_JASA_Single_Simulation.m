...
%% Basic parameters
w_L = 2200; % The length of the control filters
s_L_nofree = 1200;% The length of the secondary paths
f_L_nofree = 1200;% The length of the primary paths
SNR_en = 30; % The signal-to-noise ratio (SNR)

if signal_select==1 % Broadband noise reduction simulation
mu_CLMS = 7.1;      % The step size of the CFxLMS algorithm
mu_MDLMS = 1.8;     % The step size of the MDFxLMS algorithm
mu_MDLMS_VSR = 0.6; % The step size of the MDFxLMS-VSR algorithm
mu_BDLMS_NBC = 5.1; % The step size of the DCFxLMS and ADFxLMS-BC algorithms
mu_BDLMS = 6.2;     % The step size of the ADFxLMS algorithm

Ts = 40;    % The time length of Free-field simulations (second)
...
% Load the measured Room Impulse Responses (RIRs). 
...
load('Primary_path_DSP.mat');load('Secondary_path_1.mat');load('Secondary_path_2.mat');load('Secondary_path_3.mat');
load('Secondary_path_4.mat');load('Secondary_path_5.mat');load('Secondary_path_6.mat');load('Secondary_path_7.mat');
load('Secondary_path_8.mat');load('Secondary_path_9.mat');load('Secondary_path_10.mat');
...
for cci=1:Num_error % Primary paths in Reverberation-room simulations
PATH_P(cci,:)=Primary_path_DSP(1:f_L_nofree,cci)';
end

char_second='Secondary_path_'; % Secondary paths in Reverberation-room simulations
for eei=1:Num_error
    for ssi=1:Num_WFS_source
        file_char=num2str(ssi);
        File_Char=strcat(char_second,file_char);
        Mat_Secondary=eval(File_Char);
        PATH_S_iteration(eei,:,ssi)=(Mat_Secondary(1:s_L_nofree,eei))';
    end
end

p_error_Primary_t = (zeros(Num_error,Ts*Fs)).'; % The primary noise at error microphones
h_t_vis_error=PATH_P;                          
for ii=1:Num_error                    
     p_error_Primary_t(:,ii)=filter((h_t_vis_error(ii,:))',1,exp_Sequence);   
end 

G_L_M_J=PATH_S_iteration; % Secondary paths in Reverberation-room simulations


%% Algorithm parameters
L_Wn = w_L;                                       % The length of the control filters
d_M_n = p_error_Primary_t;                        % Primary Noise
xF_n = zeros(s_L_nofree,1);                       % Reference signal which is used to calculate the filtered-reference signal 
...
Wn_DC = zeros(L_Wn,Num_WFS_source);       % The decentralized control filters
...
An_MD = zeros(L_Wn,Num_WFS_source);       % The adaptation result
Wn_MD = zeros(L_Wn,Num_WFS_source);       % The combination result
...
An_MD_VSR = zeros(L_Wn,Num_WFS_source);       % The adaptation result
Wn_MD_VSR = zeros(L_Wn,Num_WFS_source);       % The combination result
Wn_Global_MD_VSR=zeros(Num_WFS_source*w_L,1); % The global control filter

N_VSR = 1; % The number of offline experiments for Wiener solutions calculation
...
[~, ~, Sigma_w_k]=Calculate_user_coef(Num_WFS_source,w_L,A_narrow,t_Sequence,hh,PATH_P,s_L_nofree,N_VSR,Num_error,G_L_M_J,G_VSR,b_l_k);
end
...
for ii=1:Num_WFS_source % Update the global control filter
    ...
    Wn_Global_GD_ATCC((ii-1)*L_Wn+1:ii*L_Wn,1)=Wn_G_ATCC{ii,1}{1,jj}; 
    end
    end
end
...
for adaptG_ii=1:Num_WFS_source % Step. 1 - Neighborhood-wide adaptation 
    ...
    R_Adapt_ii_AD=zeros(1,sum(Link_Matrix(:,adaptG_ii))*w_L);
        ...
        for G_jj=1:sum(Link_Matrix(:,adaptG_ii))
            Adapt_phase_ii((G_jj-1)*w_L+1:(G_jj)*w_L,1)=An_G_ATCC{adaptG_ii,1}{1,G_jj};
            ww=An_G_ATCC{adaptG_ii,1}{2,G_jj};
            
                R_Adapt_ii_AD(1,(G_jj-1)*w_L+1:(G_jj)*w_L)=...
                R_n_nomal((adaptG_ii-1)+1,((ww-1)*w_L+1):(ww*w_L));
...
for ii=1:Num_WFS_source % Update the global control filter
    ...
    Wn_Global_GD((ii-1)*L_Wn+1:ii*L_Wn,1)=Wn_G{ii,1}{1,jj}; 
    end
    end
end
...
En_G(:,sim_kk) = D_n+awgn(Yn_G,SNR_en,'measured'); % Calculate the error signal
    ...
    for adaptG_ii=1:Num_WFS_source % Step. 1 - Neighborhood-wide adaptation
        ...
        for G_jj=1:sum(Link_Matrix(:,adaptG_ii))
            Adapt_phase_ii((G_jj-1)*w_L+1:(G_jj)*w_L,1)=An_G{adaptG_ii,1}{1,G_jj};
